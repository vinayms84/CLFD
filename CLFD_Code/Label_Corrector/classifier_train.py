...
def inject_noise_uniform(e, noise_rate = 0.45):
  for i in range(e.shape[0]):
    if np.random.uniform(low=0.0, high=1.0, size=None)< noise_rate:
      e[i] = 1-e[i]
  return e

def inject_noise_class_dependent(e, noise_rate_malicious = 0.3, noise_rate_normal = 0.45):
  for i in range(e.shape[0]):
    if e[i] == 1:
      if np.random.uniform(low=0.0, high=1.0, size=None)< noise_rate_malicious:
        e[i] = 1-e[i]
    if e[i] == 0:
      if np.random.uniform(low=0.0, high=1.0, size=None)< noise_rate_normal:
        e[i] = 1-e[i]
  return e

data_vector, label = create_fine_tune_dataset(seq_length, features)
noisy_label = inject_noise_uniform(label)
#noisy_label = inject_noise_class_dependent(label)
dataset = Seq_Dataset(data_vector,noisy_label)
